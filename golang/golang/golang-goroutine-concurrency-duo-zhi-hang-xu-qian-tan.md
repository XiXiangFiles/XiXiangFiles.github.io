...
> **a goroutine memory**  
> Goroutine 可以在os 記憶體中啟到1G的數量，換句話說Java thread在同樣1G的條件最多只能執行1024 mb/1 mb條，而Goroutine則可執行\(1024kb\*1024kb\)/2kb條執行緒

### Golang 多執行緒使用方式

在Golang 中使用Goroutine 只要在 func前面加上 **“ go”關鍵字**就可以直接啟動執行序。**一般來說golang 執行緒會隨著父親死亡而跟著release**。如下範例程式:

{% code title="Goroutine 基本用法" %}
```go
package main
...
![Goroutine &#x57FA;&#x672C;&#x7528;&#x6CD5;&#x7A0B;&#x5F0F;&#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/560/1*QkO_TiZt1in5kRrc25D4Zw.png)
...
{% code title="example: 多執行緒相互溝通\(使用channel\)" %}
```go
package main
...
![example &#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/559/1*Vl-2ksuXnQT6SkTQklefeQ.png)
...
![example 2 &#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/564/1*2BH45e31RgB5EngJ8zr15Q.png)
...
{% code title="example 3: 等待一執行緒結束後再接續工作\(使用sync.WaitGroup\)" %}
```go
package main
...
![example 3&#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/563/1*_7dOlNTrqC5sAOEOd44_6g.png)

Channel 的作法是利用**等待提取、等待可注入**會lock住的特性，達到**Sync.WaitGroup** 的功能。

{% code title="example 4: 等待一執行緒結束後再接續工作\(使用channel\)" %}
```go
package main
...
![example 4 &#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/568/1*8tSXGhBXiAa84n5eEsXORg.png)

### 3. 多執行緒共用同一個變數

在多執行緒的世界，只是讀取一個共用變數是不會有問題的，但若是要進行修改可能會因為多個執行緒正在存取造成concurrent 錯誤。若要解決這種情況，必須在存取時先將資源lock住，就可以避免這種問題。

{% code title="example 5: 多執行緒共用同一個變數" %}
```go
package main
...
![ example 5&#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/571/1*5iFeuySg3bwnSlJb1dD9Yg.png)
...
{% code title="example 6: 不同執行緒產出影響後續邏輯" %}
```go
package main
...
![example 6&#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/558/1*AaMl_ldz0A-aj70ZJrEBaA.png)
...
![example 7 &#x57F7;&#x884C;&#x7D50;&#x679C;](https://miro.medium.com/max/561/1*HlQ6vJ1LlEPEanbvy_2sJw.png)
